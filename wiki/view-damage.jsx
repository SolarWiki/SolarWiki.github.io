/* Pokémon Solar Eclipse — Damage Calculator. window.VIEWS.Damage */
...
(function () {
  ...
  function MonPicker({ value, onChange, label, accent }) {
    ...
    return (
      <div style={{ position: 'relative' }}>
        ...
        {open && (
          <div style={{ position: 'absolute', zIndex: 30, top: '100%', left: 0, right: 0, marginTop: 6, background: '#0c0a05', border: '1px solid #2a2110', borderRadius: 12, padding: 10, boxShadow: '0 20px 50px #000a' }}>
            <input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Search…" spellCheck={false}
              style={{ width: '100%', padding: '8px 12px', borderRadius: 8, background: '#0f0b04', border: '1px solid #2a2110', color: '#ece3d2', fontFamily: "'Outfit', sans-serif", fontSize: 14, outline: 'none', marginBottom: 8 }} />
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(86px, 1fr))', gap: 6, maxHeight: 280, overflowY: 'auto' }}>
              {list.map(d => (
                <button key={d.dex} onClick={() => { onChange(d.dex); setOpen(false); setQ(''); }} style={{ cursor: 'pointer', padding: 6, borderRadius: 8, background: d.dex === value ? '#2a1c08' : 'transparent', border: '1px solid #241d10', textAlign: 'center' }}>
                  <SpriteSlot dex={d.dex} name={d.name} size={48} accent={TYPES[d.types[0]].glow} />
                  <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, color: '#cbbd9f', marginTop: 3, lineHeight: 1.1 }}>{d.name}</div>
                </button>
              ))}
            </div>
          </div>
  ...
  function Field({ label, children }) {
    return (
      <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}>
        <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 8, color: '#8a7d63', letterSpacing: 0.5 }}>{label}</span>
        {children}
      </label>
    );
  }
  const selStyle = { padding: '6px 10px', borderRadius: 7, background: '#0f0b04', color: '#ece3d2', border: '1px solid #2a2110', fontFamily: "'Outfit', sans-serif", fontSize: 13, minWidth: 0 };
        ...
        <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
          ...
          <Field label="NATURE">
            <select value={f.nature} onChange={e => set({ ...f, nature: e.target.value })} style={selStyle}>
              {NATURE_NAMES.map(n => <option key={n} value={n} style={{ background: '#0f0b04' }}>{n}</option>)}
            </select>
          </Field>
          <Field label="ITEM">
            <select value={f.item} onChange={e => set({ ...f, item: e.target.value })} style={selStyle}>
              {ITEMS.map(n => <option key={n} value={n} style={{ background: '#0f0b04' }}>{n}</option>)}
            </select>
          </Field>
          <Field label="ABILITY">
  ...
  function Stat({ label, value, sub, color, small }) {
    return (
      <div style={{ padding: 14, borderRadius: 12, background: '#0c0a05', border: '1px solid #241d10', textAlign: 'center' }}>
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 7, color: '#7a6c4a', letterSpacing: 0.5, marginBottom: 7 }}>{label}</div>
        <div style={{ fontFamily: "'Cinzel', Georgia, serif", fontWeight: 700, fontSize: small ? 18 : 26, color, lineHeight: 1.1 }}>{value}</div>
        {sub && <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: '#7a6c4a', marginTop: 4 }}>{sub}</div>}
      </div>
    );
  }
})();
