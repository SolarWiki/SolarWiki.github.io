/* Pokémon Solar Eclipse — Team Builder & Coverage Analyzer. window.VIEWS.Team */
window.VIEWS = window.VIEWS || {};
(function () {
  const { DEX, TYPES, STAT_LABELS } = window.VSEDEX;
  const byDex = (d) => window.VSEDEX.byDex[String(d).padStart(3,'0')] || window.VSEDEX.byDex[d];
  ...
  const LS_KEY = 'solar_eclipse_team_v2';
  const OLD_KEY = 'solar_eclipse_team_v1';
      ...
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 120, background: 'rgba(5,3,12,0.82)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '50px 20px', overflowY: 'auto' }}>
        <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 560, background: 'radial-gradient(ellipse at 30% 0%, #14100a, #0a0818 70%)', border: '1px solid #2a2110', borderRadius: 18, padding: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            ...
            <span style={{ marginLeft: 'auto', fontFamily: "'Space Mono', monospace", fontSize: 13, color: sel.length === 4 ? '#ffd54a' : '#8a7d63' }}>{sel.length}/4 moves</span>
          </div>
          <input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Search this Pokémon's moves…" spellCheck={false}
            style={{ width: '100%', padding: '10px 14px', borderRadius: 10, background: '#0f0b04', border: '1px solid #2a2110', color: '#ece3d2', fontFamily: "'Outfit', sans-serif", fontSize: 14, outline: 'none', margin: '8px 0 14px' }} />
          {moves.length === 0 ? <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: '#8a7d63', padding: '10px 0' }}>No learnable moves listed for this Pokémon yet.</div> : (
      ...
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 120, background: 'rgba(5,3,12,0.82)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '50px 20px', overflowY: 'auto' }}>
        <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 560, background: 'radial-gradient(ellipse at 30% 0%, #14100a, #0a0818 70%)', border: '1px solid #2a2110', borderRadius: 18, padding: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            <SpriteSlot dex={dex} name={mon ? mon.name : dex} size={40} accent={mon ? TYPES[mon.types[0]].glow : '#ffb347'} />
            <span style={{ fontFamily: "'Cinzel', Georgia, serif", fontWeight: 700, fontSize: 22, color: '#fff' }}>{mon ? mon.name : dex}</span>
            <span style={{ marginLeft: 'auto', fontFamily: "'Space Mono', monospace", fontSize: 12, color: evLeft < 0 ? '#ff8fa6' : (evLeft === 0 ? '#ffd54a' : '#8a7d63') }}>EVs: {evTotal}/{EV_TOTAL}</span>
          </div>

          {/* nature */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: '#8a7d63', minWidth: 56 }}>Nature</span>
            <select value={nature} onChange={e => setNature(e.target.value)}
              style={{ flex: 1, padding: '8px 10px', borderRadius: 8, background: '#0f0b04', border: '1px solid #2a2110', color: '#ece3d2', fontFamily: "'Outfit', sans-serif", fontSize: 13, outline: 'none' }}>
              {NATLIST.map(n => {
                const info = NAT[n] || {};
          ...
          {abilPool.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: '#8a7d63', minWidth: 56 }}>Ability</span>
              {abilPool.length === 1 ? (
                <span style={{ flex: 1, padding: '8px 10px', borderRadius: 8, background: '#0f0b04', border: '1px solid #2a2110', color: '#9a8d6f', fontFamily: "'Outfit', sans-serif", fontSize: 13 }}>{abilPool[0]}{mon && mon.hidden && mon.hidden.toLowerCase() === abilPool[0].toLowerCase() ? ' (hidden)' : ''}</span>
              ) : (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: "'Space Mono', monospace", fontSize: 10, color: '#7a6c4a', margin: '6px 2px 0', paddingLeft: 102 }}>
  ...
  window.VIEWS.Team = function Team() {
    ...
    const Chip = ({ t, count, tone }) => (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 9px', borderRadius: 7, background: TYPES[t].bg + '55', border: `1px solid ${TYPES[t].glow}55`, fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 600, color: TYPES[t].fg }}>
      ...
      <div>
          ...
          {Array.from({ length: MAXTEAM }).map((_, i) => {
            ...
            return (
              <div key={i}
                ...
                <div style={{ fontFamily: "'Cinzel', Georgia, serif", fontWeight: 700, fontSize: 16, color: '#fff', marginTop: 2 }}>{m.name}</div>
        ...
        {mons.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, marginTop: 14 }}>
            {/* DEFENSE */}
            <section style={{ padding: 20, borderRadius: 16, background: '#0c0a05', border: '1px solid #241d10' }}>
            ...
            <section style={{ padding: 20, borderRadius: 16, background: '#0c0a05', border: '1px solid #241d10' }}>
              ...
              <p style={{ margin: '0 0 14px', fontFamily: "'Outfit', sans-serif", fontSize: 12, color: '#8a7d63', lineHeight: 1.5 }}>Types your party hits super-effectively using its own typing.</p>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 18 }}>
                {covered.length === 0 ? <span style={{ color: '#7a6c4a', fontFamily: "'Space Mono', monospace", fontSize: 12 }}>None</span> : covered.map(t => <Chip key={t} t={t} />)}
              </div>
